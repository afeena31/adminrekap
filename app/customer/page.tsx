...
function HomePageInner() {
  ...
  const initialNewCustomer = searchParams.get("newCustomer") === "1";
  ...
  // Datang dari "New Customer" di Dashboard (?newCustomer=1) — dulu tombol
  // ini cuma Link ke "/" polos, jadi malah membuka profil customer yang
  // sedang aktif (bukan bikin profil baru seperti janji labelnya). Sekarang
  // langsung buka form "Tambah Customer Baru".
  useEffect(() => {
    if (initialNewCustomer) setNewCustomerOpen(true);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
